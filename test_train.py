...
params = {'pt_pm_lr': 0.0001, 'seed': 17204, 'ft_lr': 1e-05, 'ft_sp_weight': 3, 'pt_pm_sp_weight': 0.01, 'ft_num_epochs': 350, 'pt_pm_num_epochs': 450, 'n_repeated': 10, 'ft_update_interval': 80, 'ft_cl_weight': 0.01, 'ft_pl_weight': 0.5}
print('params',params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--direction', type=str, default='/home/guest/workplace/hxz/SURER-main/data/datasets/', help='direction of datasets')
    parser.add_argument('--dataset_name', type=str, default='Dermatology1', help='The dataset used for training/testing')
    ...
    fp.write("Av_Best: {:.4f}\t{:.4f}\n\n".format(np.mean(all_ACC), np.max(all_ACC)))
    fp.write("NMI_Best: {:.4f}\t{:.4f}\n\n".format(np.mean(all_NMI), np.max(all_NMI)))
    fp.write("Purity_Best: {:.4f}\t{:.4f}\n\n".format(np.mean(all_Purity), np.max(all_Purity)))
    fp.write("ARI_Best: {:.4f}\t{:.4f}\n\n".format(np.mean(all_ARI), np.max(all_ARI)))
